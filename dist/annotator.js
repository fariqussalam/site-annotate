...
(function () {
    ...
    function toMarkdown(annotations) {
        const lines = ['# Site Annotate — Feedback\n'];
        annotations.forEach((a, i) => {
            const el = a.element;
            lines.push(`## ${i + 1}. ${escapeMD(el.selector)}\n`);
            lines.push(`**Comment:** ${a.comment || '(no comment)'}\n`);
            lines.push(`**Element:** \`<${el.tag}>\` ${el.id ? `\`#${el.id}\`` : ''} ${el.classes.length ? `\`.${el.classes.join('.')}\`` : ''}\n`);
            if (el.text)
                lines.push(`**Text:** "${escapeMD(el.text)}"\n`);
    ...
    function cancelSelectMode() {
        ...
        (_a = document.getElementById(`${NS}-overlay`)) === null || _a === void 0 ? void 0 : _a.classList.remove('active');
        (_b = document.getElementById(`${NS}-btn-pin`)) === null || _b === void 0 ? void 0 : _b.classList.remove('active');
        document.body.style.cursor = '';
        clearHover();
    }
    function clearHover() {
        const hover = document.querySelector(`.${NS}-hover`);
        if (hover)
    ...
    function handleOverlayMove(e) {
        ...
        const hover = document.querySelector(`.${NS}-hover`);
        const label = document.querySelector(`.${NS}-hover-label`);
        if (!hover || !label)
            return;
    ...
    function handleOverlayClick(e) {
        ...
        if (ov)
            ov.style.pointerEvents = '';
        if (!target || target.closest(`.${NS}-toolbar`) || target.closest(`.${NS}-modal`) || target === ov || target.tagName === 'HTML')
            return;
        e.preventDefault();
        e.stopPropagation();
        cancelSelectMode();
    ...
    function handleDocClick(e) {
        ...
        if (!(target instanceof HTMLElement))
            return;
        if (target.closest(`.${NS}-toolbar`) || target.closest(`.${NS}-modal`) || target.closest(`.${NS}-overlay`))
            return;
        if (target.tagName === 'HTML' || target === document.documentElement)
            return;
        e.preventDefault();
        e.stopPropagation();
        cancelSelectMode();
    ...
    function openAnnotationModal(target, existing = null) {
    ...
    function updateBadge() {
        ...
        if (badge)
            badge.textContent = String(state.annotations.length);
    }
    function toggleMarkers() {
        state.markersVisible = !state.markersVisible;
        const btn = document.getElementById(`${NS}-btn-toggle`);
        if (btn)
            btn.innerHTML = state.markersVisible ? ICONS.eye : ICONS.eyeOff;
        renderMarkers();
    }
    function handleCopy() {
    ...
    // Init
    function init() {
        if (document.getElementById(`${NS}-toolbar`))
            return;
        injectStyles();
